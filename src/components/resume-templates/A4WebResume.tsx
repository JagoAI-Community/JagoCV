...
export default function A4WebResume({ data }: Props) {
  ...
  return (
    <div className="min-h-screen bg-slate-200 py-10 flex justify-center items-start overflow-auto">
      ...
      */}
      <div className="w-[210mm] min-h-[297mm] bg-white shadow-2xl flex flex-row shrink-0 mx-auto overflow-hidden transform origin-top md:scale-100 scale-75">
        
        {/* Kolom Kiri: Sidebar */}
        <aside className="w-[33%] bg-primary-900 text-white flex flex-col pt-10 pb-8 px-6">
          {/* Bagian Area Profil */}
          ...
          <div className="text-center">
            ...
            <h2 className="text-sm font-medium text-secondary-400">{data.profile.headline}</h2>
          </div>

          {/* Bagian Area Kontak */}
          <div className="space-y-3 border-b border-primary-700 pb-5 mb-6 mt-6 text-xs text-primary-50">
            {contacts.map((contact, idx) => {
              const Icon = contact.icon;
              return (
              <div key={idx} className="flex items-center gap-3">
                <Icon size={15} className="text-secondary-400 shrink-0" />
              ...
              <div className="space-y-3.5">
                {skillsList.map((group) => (
                  <div key={group.category}>
                    <h4 className="text-[11px] font-semibold text-secondary-300 mb-1.5 leading-none">
                      {group.category}
                    </h4>
                    <div className="flex flex-wrap gap-1.5">
                      {group.items.map((item) => (
                        <span
                          key={item}
                          className="px-2 py-0.5 bg-primary-800 text-primary-50 border border-primary-700 rounded-full text-[10px] whitespace-nowrap shadow-sm"
                        >
                          {item}
                        </span>
                      ))}
        ...
        <main className="w-[67%] bg-white text-gray-900 pt-10 pb-8 px-8 flex flex-col shrink-0">
          ...
          {data.experience && data.experience.length > 0 && (
            <section className="mb-7">
              <h2 className="text-lg font-bold uppercase tracking-widest text-primary-900 border-b-2 border-secondary-400 pb-1 mb-5 inline-block">
                Pengalaman Profesional
              </h2>
              <div className="space-y-6">
                {data.experience.map((exp, idx) => (
                  <div key={idx} className="relative pl-5 border-l-2 border-primary-300 text-sm">
                    {/* Titik indikatif vertikal */}
                    <div className="absolute w-2.5 h-2.5 bg-secondary-400 rounded-full -left-[5px] top-1.5 border-2 border-white"></div>
            ...
            <section className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {data.projects.map((proj, idx) => (
                  ...
                  >
                    <h4 className="font-bold text-primary-900 text-sm mb-0.5 leading-tight">{proj.name}</h4>
                    <a
                      href={proj.url.startsWith('http') ? proj.url : `https://${proj.url}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[11px] text-secondary-600 hover:text-secondary-700 underline mb-2.5 break-all inline-block font-medium"
                    >
          ...
          {data.availability && (
            <aside className="mt-auto border-l-4 border-secondary-500 bg-secondary-50 p-4 rounded-r shadow-sm">
